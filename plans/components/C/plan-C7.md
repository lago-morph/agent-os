...
- **TASK-04:** Author anchor maintainer docs for the highest-leverage custom components — kopf operator (ADR 0006 subchart), platform SDK (`memory.*`/`rag.*`), agent base images (`langgraph`/`deep-agents`, ADR 0019), Crossplane Compositions (ADR 0041 connection-secret) — produces: maintainer doc pages — depends-on: [TASK-02, TASK-03].
