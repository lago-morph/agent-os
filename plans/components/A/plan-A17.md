...
- **TASK-06:** OpenSearch MCP — system-mediated (reuse `XSearchIndex`) + external-credentialed mode — produces: OpenSearch MCPServer(s) — depends-on: [TASK-01, TASK-02]. (Resolve the `[PROPOSED]` system-mediated `authMode` enum with B13.)
- **TASK-07:** `XAgentDatabase` claims for Postgres + MongoDB (compose `XPostgres`/`XMongoDocStore`); wire MCP servers to the connection secret — produces: DB MCPServers + AgentDatabase claims — depends-on: [TASK-01, TASK-02]. (Mock B4 XRDs until landed.)
...
- **B4 (Crossplane Compositions)** — owns `XAgentDatabase`/`XPostgres`/`XMongoDocStore`/`XSearchIndex`. `[PROPOSED]` hard dep (not in CSV upstream). Mockable until landed.
