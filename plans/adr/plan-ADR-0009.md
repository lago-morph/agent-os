...
- TASK-04: Specify dual-mode `XSearchIndex` uniform-connection-secret check (kind vs AWS) — produces: Chainsaw (B4) — depends-on: [TASK-01]
...
- B4 `XSearchIndex` Composition; B6 `rag.*` API; ADR 0034 audit pipeline; ADR 0011 test-results streamer; B14 test framework.
...
Fixtures: stub audit/RAG/test-result adapters until A18/B6 land; fake `XSearchIndex` Compositions for kind/AWS.
