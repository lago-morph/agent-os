...
- TASK-04: Verify XRD claim-shape change ships conversion webhooks + deprecation windows on BOTH kind and AWS Compositions (ADR 0041) — produces: B4 dual-Composition check — depends-on: [TASK-03]
...
- ADR 0031 (CloudEvent taxonomy), ADR 0029 (JWT schema lockstep), ADR 0041 (XRD dual-Composition); backlog §1.18 (platform-release calendar + webhook/matrix tooling — deferred); B22 threat model.
...
- AC-04 → Chainsaw: XRD claim-shape change with conversion webhooks on both kind + AWS Compositions.
...
### 6.3 Merge order — independent of sibling ADR specs; coordinate with ADR-0029 (JWT lockstep) and ADR-0031/0041 (event + XRD versioning); rolls up to main.
