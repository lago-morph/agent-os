...
Enforcement/verification map. ADR 0020 is enforced primarily by **A17** (registers the fixed eight-service set as `MCPServer` CRDs, wires ESO secrets, exercises the three credential modes), over **A1** (LiteLLM as the single MCP/OAuth broker + shared callback chain), **B13** (kopf operator reconciling the `MCPServer` CRDs), **A7** (OPA admission + access-layer gating for web search/scrape and database assignment), and **B4** (`AgentDatabase`/`Postgres`/`MongoDocStore`/`SearchIndex` Compositions). **A11** is the system-mediated OpenSearch target; **A16/A14/B10** compose against the set via CapabilitySet. Conformance is tested by proving all eight services registered (no Firecrawl), the three credential modes, RBAC+OPA database assignment, access-layer policy on web services, the uniform `AgentDatabase` XR across substrates, and no gateway-side bypass.
...
- TASK-04: Verify `AgentDatabase` composes `Postgres`/`MongoDocStore` per substrate with one XR schema (ADR 0044) — produces: B4 XRD trace — depends-on: [TASK-01]
...
- ADR 0044 (substrate abstraction), ADR 0040 (Kargo promotes XRs), ADR 0038 (policy simulator), B22 threat model.
...
- AC-03 → Chainsaw: OpenSearch MCP reaches platform instance (system-mediated via `SearchIndex`) and external (credentialed).
- AC-04 → Chainsaw: `AgentDatabase` XR provisions db+role+grants composing `Postgres`/`MongoDocStore`; same shape kind/AWS.
...
### 6.3 Merge order — independent of sibling ADR specs; rolls up to main. Coordinate with ADR 0044 (substrate) + ADR 0013 (capability) specs.
...
Decision record; back out by reverting spec+plan. If reverted, A17 loses the fixed-set + credential-mode contract and B4 loses the `AgentDatabase` runtime-state precedent; per-service detail remains design-time (architecture-backlog §1.11) regardless. No runtime artifact is deleted. Removing a service means deleting its `MCPServer` CRD + wiring; agents lose it via CapabilitySet re-resolution.
