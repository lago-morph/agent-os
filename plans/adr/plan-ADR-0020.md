...
- TASK-04: Verify `XAgentDatabase` composes `XPostgres`/`XMongoDocStore` per substrate with one XR schema (ADR 0044) — produces: B4 XRD trace — depends-on: [TASK-01]
...
- AC-04 → Chainsaw: `XAgentDatabase` XR provisions db+role+grants composing `XPostgres`/`XMongoDocStore`; same shape kind/AWS.
