...
| **D-07** | 3 CRDs referenced but absent from Canon glossary | C1 (consistency-existence) | HIGH | `SearchIndex`, `MongoDocStore`, `ObjectStore` are used throughout specs but are not in `glossary.md` (likely B4 substrate XRD claim-forms). | Add them to the glossary (as B4 substrate kinds) **or** rename references to existing kinds. *(You deferred this Canon edit to your ruling.)* |
