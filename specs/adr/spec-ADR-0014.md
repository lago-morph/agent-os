...
Upstream consumed: **A11** OpenSearch (the retrieval layer governed here); **B4** Crossplane Compositions (provide `XPostgres` primaries). Downstream consumers / conformers: **A10** Letta (Postgres primary, derived OpenSearch indexes), **A18** audit pipeline, **B11** memory adapter, **C8** KB indexing, **F2** DR testing.
...
- REQ-ADR-0014-07: Consuming components MUST depend only on Postgres connection-string semantics, not on hosting shape; the kind and AWS `XPostgres` Compositions MUST write the same connection-secret keys.
