...
- `platform.audit.*` — every CR admit/deny (the §6.6 "Kubernetes admission via Gatekeeper" audit point), via the audit adapter.
...
| OPA/Rego integration | applicable — A7 **is** the engine; ships the dry-run contract + substrate-claim constraint; content from B3/B16 |
...
- **AC-A7-07** (REQ-A7-07): On a `platform.io/environment=kind` cluster, a claim whose only Composition targets `aws` is rejected at admission. *(Chainsaw)*
