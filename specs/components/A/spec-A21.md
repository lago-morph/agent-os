...
- **Tenant-scoped quotas** (max agents, sandboxes, virtual keys, cost budget, rate limits) — deferred (architecture-backlog §1.2); will compose onto the XRD when designed (ADR 0037).
...
- **Emitted:** tenant onboarding / namespace-association-change / offboarding events under **`platform.tenant.*`** (the namespace covering "tenant onboarding, namespace association changes" — interface-contract §2). Audit-emission of onboarding/offboarding actions under **`platform.audit.*`** (ADR 0034). `[PROPOSED — not in source]` for any concrete event-type name.
- **Consumed:** none required for core reconcile (Crossplane reconciles the claim; the editor reads Git synchronously). The operator offboarding notification rides the standard Headlamp suggestion-card / Mattermost channel (ADR 0037) — an existing flow, not a new A21 trigger.
