...
> upstream: [A1, A7, B13] · downstream: [] · adrs: [0009, 0013, 0014, 0020, 0021, 0030, 0031, 0033, 0034, 0038, 0040, 0044] · views: [6.1, 6.8]
...
- **A1 (LiteLLM)** — the single broker for MCP credentials and OAuth flows; A17's services route through the gateway's MCP path and share its callback chain for audit, OPA, and Langfuse. Consumes A1's `system-mediated` OAuth brokering (LiteLLM holds per-tenant OAuth credentials) and dynamic-registration boundary.
...
**`MCPServer`** (namespaced; owner B13; consumed here as instances). Source-stated fields (interface-contract §1.4): `endpoint`, `authMode` (`system`/`system-mediated`; the retired `user-cred` value is not supported, D-01), `credentialsRef`, `tags`, `scopes`, `visibility`. A17 authors instances; it does **not** invent fields.
...
- **Connection secret** (ADR 0044 uniform contract) written by `AgentDatabase` → `Postgres`/`MongoDocStore` Compositions: `host`, `port`, `user`, `password`, `dbname` (or per-primitive equivalent). A17 consumes this secret to wire the Postgres/MongoDB MCP servers; it does not redefine the shape.
...
- **OpenSearch / Postgres / MongoDB MCP** — **config + wrap** around the platform's own substrate primitives via `AgentDatabase`/`SearchIndex`; DB MCP servers themselves may be OSS or thin custom (`[PROPOSED]`, deferred).
...
- Rationale: LiteLLM is the single credential/OAuth broker (ADR 0020) so per-service code stays thin; substrate abstraction (ADR 0044) keeps DB provisioning uniform across kind/AWS.
...
- 3-layer tests — **applicable** (Chainsaw: MCPServer + AgentDatabase reconcile/admission; PyTest: web-search/scrape + OPA gating logic; Playwright: Headlamp capability view).
...
- ADR 0020 (initial MCP services set — the seven services, AgentDatabase, MCP-server-access, Firecrawl removed; credential modes `system`/`system-mediated` per D-01). ADR 0013 (capability CRD / CapabilitySet). ADR 0009 / 0033 (OpenSearch dual-hosting). ADR 0014 (Postgres primary / DB assignment gating). ADR 0021 / 0044 (XR composition / substrate abstraction / connection secret). ADR 0038 (policy-simulator target for search/scrape bundle). ADR 0040 (Kargo promotes XRs). ADR 0034 (audit). ADR 0030 / 0031 (versioning / event taxonomy).
