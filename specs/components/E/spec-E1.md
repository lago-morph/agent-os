...
- Open question (low): does the install/configure module use a dedicated throwaway cluster per trainee, or a shared cluster with namespace-scoped install exercises? §12.1 item 3 implies whole-component installs which are cluster-scoped — `[PROPOSED]` resolution: a per-trainee ephemeral kind substrate (per ADR 0041 substrate label) for install drills, shared platform for the rest. Needs confirmation with the platform-ops owner.
