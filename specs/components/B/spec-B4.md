...
> upstream: [] · downstream: [A18, A21, A23, B19, B11] · adrs: [0041, 0013, 0032, 0034, 0020, 0021, 0025, 0030, 0002] · views: [6.3, 6.6, 6.8, 6.12]
...
### 2.1 In scope
- The four **substrate XRDs** committed for v1.0 (§14, §6.3, interface-contract §1.6): `XPostgres`,
  `XSearchIndex`, `XObjectStore`, `XMongoDocStore` — each with **one Composition per substrate**
...
- The **higher-level XRDs** that compose those substrate primitives: `AuditLog` (XR `XAuditLog`,
  ADR 0034), `XAgentDatabase` (claim `AgentDatabase`, ADR 0020), `GrafanaDashboard`
  (XR `XGrafanaDashboard`, ADR 0021), `TenantOnboarding` (ADR 0028/0037).
...
- The **substrate-selection mechanism**: `platform.io/environment=kind|aws` label-driven
  ...
  with no matching Composition (ADR 0002 / 0041 — B4 owns the *requirement*; the Rego content
  is authored in B16).
- XRD-level **CRD/API versioning** wiring (ADR 0030): conversion webhooks + deprecation windows
  applied identically to XRDs when a claim shape changes.
...
- **Kargo promotion fabric** → A23 (promotes claims B4 defines).
...
- **Resource sizing / replicas / region / cluster bootstrap / Knative sources** → intentionally
  NOT wrapped (ADR 0041 documented exceptions); Kustomize overlays / install-time config.
...
config per ADR 0041, NOT wrapped here).

**Downstream consumers (HARD):**
- **A18 (audit endpoint + adapter)** — consumes `AuditLog`/`XAuditLog`, which composes `XPostgres`
  + `XObjectStore` + an indexer.
- **A21 (tenant onboarding reconciler)** — consumes `TenantOnboarding`.
- **A23 (Kargo)** — promotes B4's claims with identical shape across substrates.
...
**ADRs honored:**
- **ADR 0041** — one XRD + one Composition per substrate; uniform connection-secret shape;
...
- **ADR 0020** — `XAgentDatabase` provisions per-agent/tenant/user Postgres or MongoDB.
...
- **ADR 0030** — XRD claim-shape changes go through new `vN` + conversion webhooks + deprecation.
...
All XRDs are **namespaced claims** (the `X`-prefixed name is the cluster-scoped composite; the
user-facing claim drops the prefix — ADR 0041 / glossary). All v1.0 platform CRDs are namespaced;
...
|---|---|---|---|
| `XPostgres` ↔ `Postgres` | `version`, `size`, `storage`, `connectionSecretRef`, `substrateClass` | CloudNativePG `Cluster` | Crossplane `RDSInstance` |
| `XSearchIndex` ↔ `SearchIndex` | `version`, `nodeCount`, `storage`, `connectionSecretRef`, `substrateClass` | in-cluster OpenSearch operator | AWS-managed OpenSearch (Crossplane MR) |
| `XObjectStore` ↔ `ObjectStore` | `bucketName`, `lifecycle`, `connectionSecretRef`, `substrateClass` | MinIO (or no-op) | Crossplane S3 bucket |
| `XMongoDocStore` ↔ `MongoDocStore` | `version`, `size`, `storage`, `connectionSecretRef`, `substrateClass` | Bitnami MongoDB | DocumentDB or self-managed |
...
|---|---|---|
| `AuditLog` ↔ `AuditLog` (XR `XAuditLog`) | `postgresRef`, `s3BucketRef`, `indexerRef`, `batchScheduleSpec`, `endpointReplicas` | `XPostgres` + `XObjectStore` + indexer + (AWS) batch CronJob + endpoint Deployment |
| `XAgentDatabase` ↔ `AgentDatabase` | `engine` (postgres/mongodb), `scope` (agent/tenant/user), `ownerRef`, `credentialsSecretRef` | `XPostgres` **or** `XMongoDocStore` per `engine` |
| `GrafanaDashboard` ↔ `GrafanaDashboard` (XR `XGrafanaDashboard`) | `dashboardJson`, `folder`, `visibility` (RBAC + OPA-controlled) | Grafana folders (kind) / Crossplane-provisioned Grafana (AWS) |
...
**Status fields (every XR, substrate-agnostic — ADR 0041):** `ready`, `endpoint`, `version`.
...
N/A — B4 exposes no programmatic SDK. Its "API" is the **claim API** (the namespaced claims above)
and the **connection-secret shape** (§4.4). Consumers interact via Kubernetes claim objects and
...
**The connection-secret contract (architectural invariant, tested — ADR 0041 / interface-contract §4):**
- Each substrate XRD's **both Compositions write the same secret shape**: `host`, `port`, `user`,
  ...
  (`[PROPOSED — not in source]` — e.g. `XObjectStore` likely substitutes `bucket`/`accessKeyId`/
...
- **Capability-parity is not promised**: claim *shape* is uniform; runtime *behaviour* may differ
  (e.g. kind `XObjectStore` may produce "no archive"; kind `AuditLog` is Postgres-only). Each XRD
...
**build-new** XRDs + Compositions; **wrap** the backing operators/MRs. Rationale (ADR 0041):
Crossplane Compositions are the canonical pattern for substrate abstraction; alternatives (parallel
manifests, Kustomize-only, Helm conditionals) were rejected in ADR 0041 for drift, substrate-leak,
and broken cross-substrate promotion.

## 6. Functional Requirements
- **REQ-B4-01:** B4 MUST define the four substrate XRDs `XPostgres`, `XSearchIndex`, `XObjectStore`,
  `XMongoDocStore`, each as a **namespaced claim** with the source-stated fields (§4.1).
...
- **REQ-B4-03:** The kind Compositions MUST compose CloudNativePG (`XPostgres`), in-cluster
  OpenSearch operator (`XSearchIndex`), MinIO/no-op (`XObjectStore`), Bitnami MongoDB
  (`XMongoDocStore`); the AWS Compositions MUST compose RDS, managed OpenSearch, S3, and
  DocumentDB/self-managed Mongo respectively (ADR 0041).
- **REQ-B4-04:** B4 MUST define the `AuditLog` XRD (XR `XAuditLog`) composing `XPostgres` +
  `XObjectStore` + an OpenSearch indexer + (AWS only) a batch CronJob + the audit endpoint
  Deployment; on kind it MUST degrade to Postgres-only with the batch step disabled (ADR 0034).
- **REQ-B4-05:** B4 MUST define `XAgentDatabase` (claim `AgentDatabase`) composing `XPostgres`
  **or** `XMongoDocStore` per the declared `engine`, scoped agent/tenant/user (ADR 0020).
- **REQ-B4-06:** B4 MUST define `GrafanaDashboard` (XR `XGrafanaDashboard`) as a namespaced XR with
...
- **REQ-B4-09:** Every XR's user-visible **status MUST be substrate-agnostic** (`ready`, `endpoint`,
  `version`); substrate-specific values MUST NOT appear in user-visible status (ADR 0041).
- **REQ-B4-10:** Substrate selection MUST be **label-driven** on `platform.io/environment`; a claim
  whose target substrate has **no matching Composition** MUST be rejected at admission (B4 supplies
  the admission requirement + input shape to A7/B16; ADR 0002/0041).
- **REQ-B4-11:** XRD claim-shape changes MUST follow ADR 0030 (new `vN` API group + conversion
...
- **REQ-B4-12:** Both Compositions of each substrate XRD MUST write the **same connection-secret
  ...
  referenced by `connectionSecretRef`; this conformance MUST be tested as an invariant (ADR 0041).
- **REQ-B4-13:** Each XRD MUST **document its substrate-specific behavioural differences**
  (capability-parity caveat — e.g. kind `XObjectStore` "no archive", kind `AuditLog` Postgres-only).
- **REQ-B4-14:** B4 MUST declare/install the kind backing operators it composes (CloudNativePG,
  ...
  install-time config and MUST NOT be wrapped (ADR 0041 exception).
...
- **Scale:** Compositions are declarative; day-2 drift reconciliation is standard Crossplane
  behaviour (substrate-side console edits reconciled back to declared state — ADR 0041).
- **Versioning (ADR 0030):** per-component XRD ownership sits with B4; breaking claim-shape changes
  are coordinated because A18/A21/A23/B19/B11 bind to the claims and the secret shape.

## 8. Cross-Cutting Deliverable Checklist
- Helm/manifests — **applicable** (XRD + Composition packages; kind backing-operator installs).
- Per-product docs (10.5) — **applicable** (per-XRD claim reference, substrate-difference docs,
...
- Grafana dashboard (Crossplane XR) — **applicable** — B4 *owns* the `GrafanaDashboard` XR primitive;
  a B4 operability dashboard is itself authored as a `GrafanaDashboard` claim.
- Headlamp plugin — **N/A** — claim editing/visibility surfaces are A22 (graphical editors) / A21
...
- 3-layer tests — **applicable** (Chainsaw for claim→Composition→secret-shape; PyTest for
...
- **AC-B4-01:** Each of `XPostgres`/`XSearchIndex`/`XObjectStore`/`XMongoDocStore` installs as a
  namespaced-claim XRD with the source-stated fields. (→ REQ-B4-01)
- **AC-B4-02:** For each substrate XRD, exactly one Composition matches `platform.io/environment=kind`
  and one matches `=aws`; a claim resolves to the matching one. (→ REQ-B4-02)
- **AC-B4-03:** On kind, claims provision CloudNativePG/OpenSearch-operator/MinIO/Bitnami-Mongo; on
...
- **AC-B4-05:** An `AgentDatabase` claim with `engine=postgres` composes `XPostgres`; with
  `engine=mongodb` composes `XMongoDocStore`. (→ REQ-B4-05)
...
- **AC-B4-08:** `MemoryStore`, `AgentEnvironment`, `SyntheticMCPServer` claims install with their
...
- **AC-B4-10:** A claim submitted on a cluster whose label has no matching Composition is **rejected
  at admission** (not stuck-reconciling). (→ REQ-B4-10)
- **AC-B4-11:** A claim-shape change ships a new `vN` group with a working conversion webhook and the
...
- **AC-B4-13:** Each XRD's docs enumerate its substrate-specific behavioural differences; kind
  `XObjectStore` "no archive" and kind `AuditLog` "Postgres-only" are documented. (→ REQ-B4-13)
- **AC-B4-14:** On a fresh kind cluster, B4's prerequisite installs bring up CloudNativePG/OpenSearch
  operator/MinIO/Bitnami-Mongo before any claim is reconciled. (→ REQ-B4-14)

## 10. Risks & Open Questions
- **R1 (high):** Per-primitive connection-secret field lists beyond the five canonical keys are
  **not in source** (`[PROPOSED]`). If A18/B11 assume different key names for `XObjectStore`, the
...
- **OQ1:** Does B4 own installing the AWS provider stack, or is it purely install-time config? ADR
  0041 says provider-stack selection is install-time and NOT wrapped → B4 declares it a prerequisite,
...
- ADR 0041 (substrate abstraction — primary), 0034 (audit pipeline), 0020 (XAgentDatabase / initial
